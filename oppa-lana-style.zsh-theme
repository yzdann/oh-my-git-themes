...
function custom_build_prompt {
    ...
    if [[ $is_a_git_repo == true ]]; then
        ...
        prompt+=$(enrich_append $action "${omg_has_action_in_progress_symbol} $action" "${blue_on_white}")
            ...
            if [[ $has_upstream == false ]]; then
                ...
                if [[ $has_diverged == true ]]; then
                    ...
                    if [[ $commits_behind -gt 0 ]]; then
                        prompt+=$(enrich_append true "-${commits_behind} %F{white}${omg_can_fast_forward_symbol}%F{black} --" "${black_on_blue}")
                    fi
                    if [[ $commits_ahead -gt 0 ]]; then
                        prompt+=$(enrich_append true "-- %F{white}${omg_should_push_symbol}%F{black}  +${commits_ahead}" "${black_on_blue}")
                    fi
